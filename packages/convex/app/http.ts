...
app.post(
  ...
  async (c) => {
      ...
      case 'ACTIVE911':
        ...
        if (unitString) {
          if (unitString.length > 255) {
            return c.json(
              { error: 'Units must be less than 255 characters' },
              400
            )
          }
          units = unitString
    ...
    if (departmentId === 'ALL') {
      ...
      departmentIdExistsPromise = c.env.runQuery(
        internal.organizationSchema.departments.departmentExist,
        { id: departmentId as Id<'departments'> }
      )
    }
    const organizationIdExistsPromise = c.env.runQuery(
      internal.organizationSchema.organizations.organizationExist,
    ...
    // We let there exist an ALL departmentId for large scale integrations who might not want to
    ...
    const mappableDescriptorsPromise = c.env.runQuery(
      internal.configurationSchema.descriptors.mappableDescriptors,
      { organization: organizationId as Id<'organizations'> }
    )
    const mappableUnitsPromise = c.env.runQuery(
      internal.configurationSchema.units.mappableUnits,
      { organization: organizationId as Id<'organizations'> }
    )
    ...
    const descriptorId = mappableDescriptors.find(
      (d) => d.cadDescriptor === descriptor
    )
    if (descriptorId) {
    ...
    const response = await c.env.runMutation(
      internal.alerts.createAlertWithApiKey,
      {
        apiKey: key,
        organization: organizationId as Id<'organizations'>,
...
app.patch(
  ...
  async (c) => {
    ...
    const body = c.req.valid('json')
    const response = await c.env.runMutation(
      internal.alerts.updateAlertWithApiKey,
      {
        apiKey: key,
        organization: organizationId as Id<'organizations'>,
        department: departmentId as Id<'departments'>,
