...
function groupOfDispatch(dispatch: TransformedDispatch): RenderGroup {
...
export function IncidentMarkersRenderer() {
  ...
  const dispatchesByGroup = useMemo(() => {
    const byGroup: Record<RenderGroup, TransformedDispatch[]> = {
      fire: [],
      medical: [],
      mva: [],
      aircraft: [],
    ...
    for (const dispatch of dispatches) {
      // Dispatches whose location was removed by a transformation rule have
      // no position to render — they must not reach the clusterers.
      if (!dispatch.location) continue;
